...
// export default Products;
import React, { useState, useEffect } from "react";
import { useSearchParams, Link } from "react-router-dom"; // 👈 Added useSearchParams
import { api } from "../api";
...
function Products() {
  ...
  // --- UPDATED: Filter based on URL category ---
  ...
  return (
    <div className="container" style={{ paddingTop: "40px", paddingBottom: "80px" }}>
      ...
      <div className="section-title">
        <p className="subtitle">OUR CATALOG</p>
        <h2>
          {categoryFromUrl ? categoryFromUrl : "Explore Our Chemical Products"}
        </h2>
      </div>
      ...
      {!isLoading && filteredProducts.length === 0 && (
        <div style={{ textAlign: "center", padding: "60px 0", color: "#888" }}>
          <h3>No products found.</h3>
          {categoryFromUrl && <p>No products in the <strong>"{categoryFromUrl}"</strong> category.</p>}
          {!categoryFromUrl && <p>Check back later for new catalog items.</p>}
          <Link to="/products" style={{ marginTop: "20px", display: "inline-block", color: "#002D5A", fontWeight: "bold" }}>
            View all products →
          </Link>
        </div>
      )}
